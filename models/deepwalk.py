...
class DeepWalk(BaseModel):
    ...
    def learn_embeddings(self):
        ...
        directory_path = CURRENT_DIR.joinpath('deepwalk_' )
        Path(directory_path).mkdir()
        edgelist_path = self.__dump_graph(directory_path)

        embeddings_file = str(CURRENT_DIR.joinpath('deepwalk_embeddings_' ))
        ...
        embeddings_df = pd.read_csv(embeddings_file, sep=' ', header=None, skiprows=1, names=['id'] + [f'emb_{i}' for i in range(self.dim)])
